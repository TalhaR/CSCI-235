...
void LinkedList<T>::invert() {
   // leaves method if list is empty
...
void LinkedList<T>::invertRest(Node<T>* current_first_ptr) {
   ...
   if (current_first_ptr == nullptr) return;
   // When the method reaches the end of the list
   // it sets that point as the new head of the list
   // then returns
   ...
   // first and rest 
   first = current_first_ptr;
   rest = first->getNext();

   invertRest(rest);

   // after setting the new head pointer
   // this will start reversing the order of the 
   // list by setting the previous rest pointers 
   // to point to the first pointers
   rest->setNext(first);
...
void LinkedList<T>::rotate(int k) {
   ...
   // if k == 0 then there's no more rotations to be done so return
   // if k is a multiple of the list size then rotating it that many
   // times will just give the same list so just leave the method
   if (k <= 0 || k % item_count_ == 0 || head_ptr_ == nullptr) {
      return;
   } else {
      Node<T>* first = head_ptr_;
      Node<T>* last;
      Node<T>* current = head_ptr_;

      // moves current pointer to 2nd to last element
      while (current->getNext()->getNext() != nullptr) {
         current = current->getNext();
      }
      // sets last pointer to last element and links it to the 
      // original first element
      last = current->getNext();
      last->setNext(first);   
      // sets the 2nd to last element to nullptr since that is 
      // now going to be the last element
      current->setNext(nullptr);
      // sets the headpointer to the previously last element
      head_ptr_ = last;
      
      // recursive call to repeat this action k times
      rotate(k-1);
   }
}
